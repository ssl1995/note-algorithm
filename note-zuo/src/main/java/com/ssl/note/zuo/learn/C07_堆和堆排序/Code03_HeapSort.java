...
public class Code03_HeapSort {
    ...
    // 最小值在前面
    private static void heapify1(int[] arr, int parent, int n) {
        while (2 * parent + 1 < n) {
            int left = 2 * parent + 1;
    ...
    // 小根堆2：使用heapInsert = 从上到下
    public static void heapSort(int[] arr) {
        if (arr == null || arr.length < 2) {
            return;
        }
        // O(N*logN):使用heapInsert = 从上到下
        for (int i = 0; i < arr.length; i++) { // O(N)
            heapInsert(arr, i); // O(logN)
        }
        int heapSize = arr.length;
        // 小根堆：与末尾元素交换，让大值来到最终位置
        swap(arr, 0, --heapSize);
        // O(N*logN)
    ...
    // 新加进来的是arr[index]，依次往上看，让[0,index]是一个大根堆
