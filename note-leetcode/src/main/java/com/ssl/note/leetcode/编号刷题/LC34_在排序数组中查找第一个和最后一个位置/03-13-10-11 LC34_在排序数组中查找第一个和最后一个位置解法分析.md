...
| 变种类型 | 目标 | 核心判断条件 | 记忆口诀 |
|---------|------|-------------|---------|
| 标准二分 | 查找等于target的元素 | `nums[mid] == target` | 相等直接返回 |
| 左边界 | 查找第一个>=target的位置 | `nums[mid] < target` → left=mid+1 | 小于目标，右移左边界 |
| 右边界 | 查找最后一个<=target的位置 | `nums[mid] > target` → right=mid-1 | 大于目标，左移右边界 |
| 上界 | 查找第一个>target的位置 | `nums[mid] <= target` → left=mid+1 | 小于等于目标，右移左边界 |
| 插入点 | 查找应该插入的位置 | 同左边界逻辑 | 左边界即插入点 |
...
**应用步骤**：
1. 明确目标：是找左边界、右边界还是上界
2. 确定判断条件：根据目标选择合适的移动策略
3. 处理边界情况：数组为空、目标不存在等
4. 验证结果：确保逻辑正确
